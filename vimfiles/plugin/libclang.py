  ...
  for path in knownPaths:
    try:
      ...
      if canFindBuiltinHeaders(index, [arg]):
        print "canFindBuiltinHeaders return True"
        print arg
        print path
        return path
    except:
...
def getCurrentTranslationUnit(args, currentFile, fileName, timer,
  ...
  try:
    ...
    #  output(args, info="args=")
    #  for l in range(0,len(tu.diagnostics)):
    #    output(tu.diagnostics[l], info="diagnostic:%d"%l)
...
def getCompileParams(fileName):
  ...
  # output( vim.eval("b:clang_user_options"), info="b:clang_user_options:")
